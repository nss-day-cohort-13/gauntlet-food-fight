var FoodFight = (function (results) {
	...
	results.checkForDeath = function(currentHeroHealth, currentEnemyHealth, hero, enemy) {
		if (currentHeroHealth <= 0 || currentEnemyHealth <= 0) {
			modalDiv.modal("show");
			if(currentHeroHealth <= 0 && currentEnemyHealth <= 0) {
				resultsDiv.html(`<img id="deathImage1" value="PLAY"  onload="loseSound.play()" src = "${hero.deathImage}" width = "400"><h3>You have both been defeated. Would you like to play again?</h3>`);
			}
				...
				resultsDiv.html(`<img value="PLAY"  onload="winSound.play()"><h3>${hero.name} the ${hero.species} has beaten ${enemy.name}! Would you like to play again?`);
			}
		}
